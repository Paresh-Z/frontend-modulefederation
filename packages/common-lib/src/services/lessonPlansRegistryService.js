...
export const getAll = async ({ limit, ...params } = {}, header = {}) => {
    ...
    const result = await post(
        manifest.api_url + '/lessonPlan/search',
...
export const getOne = async (filters = {}, header = {}) => {
    ...
    try {
        const result = await get(manifest.api_url + `/lessonPlan/${filters.id}`, {
            //params: filters.id,
            headers
        })
        if (result?.data?.data) {
...
export const create = async (data, header = {}) => {
    ...
    const result = await post(manifest.api_url + '/lessonPlan', newData, {
        headers: headers?.headers ? headers?.headers : {}
    })
    if (result.data) {
        return true
...
export const update = async (data = {}, headers = {}) => {
    ...
    const result = await coreUpdate(
        manifest.api_url + '/lessonPlan/' + data.id,
        newData,
        {
            headers: headers?.headers ? headers?.headers : {}
        }
